...
public class Misc {
    ...
    /**
     * <p>Given a center point and list of other positions, calculate the average position as an offset from the center. E.g.:</p>
     * <ul>
     *     <li>center (2,4,5) ; others [] = (0,0,0)</li>
     *     <li>center (2,4,5) ; others [(2,4,9)] = (0,0,2)</li>
     * </ul>
     *
     * @param center Center point to base around.
     * @param others Other positions around the center.
     * @return Average position as an offset from center.
     */
    public static Vec3 getAverageOffsetFrom(BlockPos center, Collection<BlockPos> others) {
        var accum = new Vec3(0, 1, 0);
        if (others.isEmpty()) return accum;
        var scale = 1f / (1 + others.size());
        for (BlockPos other : others)
            accum = accum.add(Vec3.atLowerCornerOf(other.subtract(center)).scale(scale));
        return accum;
    }
}
